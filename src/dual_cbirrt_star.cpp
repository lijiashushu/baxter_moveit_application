...
//                            if(extend_num % _extend_step_num == 0 || (qs_task_state_vector - _random_state_vector).norm() < 0.01){
                                if(true) {
                                    ...
                                    int treeIndex;
                                    //***********************************ChooseParent***********************************
                                    flann_index[if_tree_a].radiusSearch(querry, indices, dist, 0.05, flann::SearchParams());
                                    ...
                                    for (int i = 0; i < indices[0].size(); i++) {
                                        ...
                                        if (minCost > dist[0][i] + _tree_joints_angle_matrix[if_tree_a][treeIndex].second) {
                                            feasibleCheckStack.push_back(std::make_pair(dist[0][i] + _tree_joints_angle_matrix[if_tree_a][treeIndex].second, i));
                                            //小步长直接连接
...
//                                            minIndex = treeIndex;
                                    ...
                                    //***********************************Add State***********************************
                                    reached_index = addState(qs_matrix, qs_slave_matrix, qs_task_state_vector, qs_slave_task_state, qs_state, minIndex, if_tree_a, minCost);
