...
void CBiRRT::sample(robot_state::RobotState & goal_state, robot_state::RobotState & random_state, Eigen::Matrix<double, 7, 1> & random_state_value_matrix, const robot_state::JointModelGroup* planning_group) {
...
size_t CBiRRT::near_a_tree(Eigen::Matrix<double, 7, 1> & random_state_value_matrix, Eigen::Matrix<double, 7, 1> & nearest_node_matrix) {
    double minum_dis = std::numeric_limits<double>::max();
    size_t minum_index = 0;
    for(size_t i=0; i<_a_rrt_tree_matrix.size(); i++){
        Eigen::Matrix<double, 7, 1> dis_matrix = random_state_value_matrix - _a_rrt_tree_matrix[i];
    ...
    nearest_node_matrix = _a_rrt_tree_matrix[minum_index];
    return minum_index;
}

size_t CBiRRT::near_b_tree(Eigen::Matrix<double, 7, 1> & random_state_value_matrix, Eigen::Matrix<double, 7, 1> & nearest_node_matrix) {
    double minum_dis = std::numeric_limits<double>::max();
    size_t minum_index = 0;
    for(size_t i=0; i<_b_rrt_tree_matrix.size(); i++){
        Eigen::Matrix<double, 7, 1> dis_matrix = random_state_value_matrix - _b_rrt_tree_matrix[i];
        double distance = dis_matrix.norm();
        if(distance < minum_dis){
            minum_dis = distance;
            minum_index = i;
...
//                    std::cout<<end_jacobian<<std::endl;
            ...
            if(project_success){
                ...
                if(planning_scene_ptr->isStateValid(qs_state, planning_group_name)) {
                    ...
                    std::cout<<std::endl;
                    _a_rrt_tree_matrix.push_back(qs_matrix);
...
void CBiRRT::constraint_extend_b_tree(Eigen::Matrix<double, 7, 1> & random_state_value_matrix, Eigen::Matrix<double, 7, 1> & nearest_node_matrix, size_t nearst_node_index, Eigen::Matrix<double, 7, 1> & reached_state_matrix, const robot_state::JointModelGroup* planning_group, const std::string & planning_group_name, planning_scene::PlanningScenePtr & planning_scene_ptr){
    size_t qs_old_index = nearst_node_index;
    Eigen::Matrix<double, 7, 1> qs_matrix = nearest_node_matrix;
    Eigen::Matrix<double, 7, 1> qs_old_matrix = nearest_node_matrix;
    Eigen::Matrix<double, 7, 1> direction_matrix;
...
//                    std::cout<<end_jacobian<<std::endl;
            ...
            if(project_success){
                ...
                if(planning_scene_ptr->isStateValid(qs_state, planning_group_name)) {
                    ...
                    std::cout<<std::endl;
                    _b_rrt_tree_matrix.push_back(qs_matrix);
                    std::pair<robot_state::RobotState, size_t> tmp(qs_state, qs_old_index);
...
bool CBiRRT::plan(robot_state::RobotState & goal_state, robot_state::RobotState & start_state, planning_scene::PlanningScenePtr& planning_scene_ptr, const std::string & planning_group_name, const robot_state::JointModelGroup* planning_group){
    ...
    _a_rrt_tree_matrix.push_back(start_value_matrix);
    _b_rrt_tree_matrix.push_back(goal_value_matrix);
    ...
    for(int count=0; count < 100000; count++){
        ...
        if(extend_order){
            ...
            constraint_extend_a_tree(random_state_value_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr);
            nearest_node_index = near_b_tree(a_tree_reached_matrix, nearest_node_matrix);
            constraint_extend_b_tree(a_tree_reached_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr);
            ...
            constraint_extend_b_tree(random_state_value_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr);
            nearest_node_index = near_a_tree(b_tree_reached_matrix, nearest_node_matrix);
            constraint_extend_a_tree(b_tree_reached_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr);
