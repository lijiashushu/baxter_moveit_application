...
*/
class JointSpacePlanner : public PathPlanner{
    ...
    //allowed deviation
    double _yaw_deviation = 1.05;
    double _pitch_deviation = 0.05;
    double _roll_deviation = 0.05;

    //算法运行过程中需要使用的状态变量
    Eigen::Matrix<double, 7, 1> _random_joint_angles;
    ...
    double _joint_step_size; //扩展时一点点扩展，但是增长了0.1m之后才算作一个新的节点，相当于这个step是collision check的step
    ...
    Eigen::Matrix<double, 7, 7> _info_C_M;
    Eigen::Matrix<double, 7, 1> _info_centre;
    double _info_cMin;
    bool _foundPathFlag;
    ...
    bool project(Eigen::Matrix<double, 7, 1> &joint_angles);
    /**
   ...
   */
    bool extend_check(bool if_tree_a, const Eigen::MatrixXd& goal_master_angles, int& try_index, double& remain_dis,
                      std::vector<flann::Index<flann::L2<double>>>& flann_index);
...
//    std::vector<std::pair<Eigen::Vector4d, Eigen::Vector4d>> _planning_result_task_state_vector;
    ...
    /**
       ...
       */
    JointSpacePlanner(const ros::NodeHandle &nh, std::string base_frame="base");
    ~JointSpacePlanner();
    ...
    /**
       ...
       *  @param  if_informed  if activate rrt* method
       */
