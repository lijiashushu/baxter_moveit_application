...
//    spinner.start();
    ...
    int constrain_index = 1;
    int seed;
    int test_times = 3;
    int max_sample_times = 500;
    double bi_probility = 0.8;
    double task_step = 0.1;
    double max_planning_time = 5.0;
    double traj_soft_constraint_weight = 2;
    double traj_grad_step = 1;
    ...
    //注意这里的experiment1是桌子，和文章里面是相反的。
    std::string outputDir = "/home/lijiashushu/ros_ws/src/dual_rrt_star/result/experiment2/";
    std::string seedFileName = "seed.txt";

    std::string waypointsNumName = "traj/weight2/waypoints_num.txt";
    std::string planningTimeName = "traj/weight2/planning_time.txt";
    std::string dataName = "traj/weight2/data/";

    std::ofstream waypointsNumFile(outputDir + waypointsNumName, ios::app);
    std::ofstream timeFile(outputDir + planningTimeName, ios::app);
    std::ifstream seedFile(outputDir + seedFileName);
    if(!seedFile){
        std::cout<<"open seed file fail!"<<std::endl;
    }
...
//        }
        ...
        planner->initPara(seed,
                          ...
                          true);
        if(planner->plan()){
            Eigen::MatrixXd pathPoints = Eigen::MatrixXd::Zero(planner->_planning_result_joint_angles.size(), 14);

            for(int i=0; i<planner->_planning_result_joint_angles.size(); i++){
                pathPoints.row(i) = planner->_planning_result_joint_angles[i];
            }
            ...
            waypointsNumFile<<pathPointsNew.rows()<<std::endl;
            Eigen::MatrixXd allSegCoeff(pathPointsNew.rows()-1, 7*(14+1));
            TimeDualConstraintPlanner dual_arm_traj_planner;
            planning_time = dual_arm_traj_planner.getAllSegCoeff(pathPointsNew, allSegCoeff, traj_soft_constraint_weight, traj_grad_step, constrain_index);
            dual_arm_traj_planner.outputTrajToFile(allSegCoeff, pathPointsNew, outputDir+dataName+std::to_string(count));
            timeFile<<planning_time<<std::endl;
        }
        else{
        ...
        count++;
        std::cout<<"traj_plan_time:  "<<planning_time<<std::endl;
    }

    seedFile.close();
    timeFile.close();
    waypointsNumFile.close();
