    ...
    while (true){
        ...
        if ((qs_matrix - random_state_value_matrix).norm() < 0.05){
            ...
            if(!(link_name =="")){
                ...
                minimum_collision_dis_jac_pinv = minimum_collision_dis_jac.transpose()*((minimum_collision_dis_jac * (minimum_collision_dis_jac.transpose()) +  0.01 * Eigen::Matrix<double,6,6>::Identity() ).inverse());
                double alpha;
...
void DualCBiRRT::constraint_extend_tree_dense_collide(Eigen::Matrix<double, 7, 1> & random_state_value_matrix, Eigen::Matrix<double, 7, 1> & nearest_node_matrix, size_t nearst_node_index, Eigen::Matrix<double, 7, 1> & reached_state_matrix, const robot_state::JointModelGroup* planning_group, const std::string & planning_group_name, planning_scene::PlanningScenePtr & planning_scene_ptr, const robot_state::JointModelGroup* slave_group, std::pair<std::vector<double>, std::vector<double>>& slave_joint_pos_bounds, PerformanceIndexOneSample & perdex_one_sample, collision_detection::CollisionWorldFCL & world_FCL, const collision_detection::CollisionRobotConstPtr & robot, bool if_tree_a){
    collision_detection::CollisionRequest collision_req;
    collision_detection::CollisionResult collision_res;
    collision_req.group_name = "both_arms";
...
//                    if ((qs_matrix - qs_old_matrix).norm() > 2*_step_size || !qs_state.satisfiesBounds(planning_group, 0.05)){
            ...
            if(project_success){
                perdex_one_extend.project_success = 1;
                ik_start_time =  ros::Time::now();
                bool ik_result = solve_IK_problem_dense_collide(current_slave_angles_matrix, qs_matrix, computed_slave_angles_matrix, planning_group, slave_group, planning_scene_ptr, slave_joint_pos_bounds, perdex_one_extend, world_FCL, robot);
                ik_end_time = ros::Time::now();
                perdex_one_extend.ik_total_spend_time = double((ik_end_time - ik_start_time).nsec) / 1000000000;
                if(ik_result){
                    perdex_one_extend.ik_success = 1;
...
bool DualCBiRRT::plan_dense_collide(robot_state::RobotState & goal_state, robot_state::RobotState & start_state, planning_scene::PlanningScenePtr& planning_scene_ptr, const std::string & planning_group_name, const robot_state::JointModelGroup* planning_group, const robot_state::JointModelGroup* slave_group){
...
//    }
    ...
    for(int count=0; count < _max_planning_times; count++){
        ...
        if(extend_order){
            ...
            nearest_node_index = near_tree(random_state_value_matrix, nearest_node_matrix, extend_order);
            constraint_extend_tree_dense_collide(random_state_value_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, extend_order);
            nearest_node_index = near_tree(a_tree_reached_matrix, nearest_node_matrix, !extend_order);
            constraint_extend_tree_dense_collide(a_tree_reached_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, !extend_order);
            ...
            nearest_node_index = near_tree(random_state_value_matrix, nearest_node_matrix, extend_order);
            constraint_extend_tree_dense_collide(random_state_value_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, extend_order);
            nearest_node_index = near_tree(b_tree_reached_matrix, nearest_node_matrix, !extend_order);
            constraint_extend_tree_dense_collide(b_tree_reached_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, !extend_order);
        ...
        if(count > 2000){
            ...
            if(std::abs(pos_error[0]) < 0.01 && std::abs(pos_error[1])< 0.01 && std::abs(pos_error[2])< 0.01 &&  euler_error.norm() < 0.03){
                ...
                if(!(link_name =="")){
                    ...
                    minimum_collision_dis_jac_pinv = minimum_collision_dis_jac.transpose()*((minimum_collision_dis_jac * (minimum_collision_dis_jac.transpose()) + 0.01 * 0.01 * Eigen::Matrix<double,6,6>::Identity() ).inverse());
                    double alpha;
                    if(minimum_dis_value > 1.0 || minimum_dis_value < 0.05){
                        alpha = 0;
                    }
...
//                D1 = svd_holder1.singularValues();
                ...
                slave_end_jacobian_mp_inverse = (slave_end_jacobian.transpose() * ((slave_end_jacobian * slave_end_jacobian.transpose() + 0.01 * Eigen::Matrix<double,6,6>::Zero()).inverse())).eval();
