...
void DualCBiRRT::constraint_extend_task_space_dir_astar(Eigen::Matrix<double, 7, 1> & random_state_value_matrix, Eigen::Matrix<double, 7, 1> & nearest_node_matrix, size_t nearst_node_index, Eigen::Matrix<double, 7, 1> & reached_state_matrix, const robot_state::JointModelGroup* planning_group, const std::string & planning_group_name, planning_scene::PlanningScenePtr & planning_scene_ptr, const robot_state::JointModelGroup* slave_group, std::pair<std::vector<double>, std::vector<double>>& slave_joint_pos_bounds, PerformanceIndexOneSample & perdex_one_sample, collision_detection::CollisionWorldFCL & world_FCL, const collision_detection::CollisionRobotConstPtr & robot, bool if_tree_a, bool if_sample, Eigen::Vector3d& reached_task_pos, size_t & reached_index, AstarPathFinder * astar_path_finder){
    ...
    //**********************************Astar 相关变量*********************************
    Eigen::Vector3d astar_start_pt;
    astar_start_pt = qs_task_state_vector;
    std::vector< Eigen::Vector3d> astar_path;
    bool astar_path_finded;
    if(if_tree_a){
        astar_path_finded = astar_path_finder->AstarGraphSearchMultiTarget(astar_start_pt, _b_rrt_tree_task_space_state);
    }
    else{
        astar_path_finded = astar_path_finder->AstarGraphSearchMultiTarget(astar_start_pt, _a_rrt_tree_task_space_state);
    }
    if(astar_path_finded){
        astar_path = astar_path_finder->getPath();
    }
    else{
        reached_state_matrix = nearest_node_matrix;
        reached_task_pos = qs_task_state_vector;//也就是 nearest 对应的 pos vector
        reached_index = nearst_node_index;
    ...
    int performance_index_extend_num = 0;
    int extend_num = 0;
    auto path_it = astar_path.begin();
    std::cout<<"*path_it.size()  "<<astar_path.size()<<std::endl;
    std::cout<<"*path_it  "<<(*path_it).transpose()<<std::endl;
    random_task_state_vector = *path_it;
    while (true){
        extend_num++;
        std::cout<<"extending"<<std::endl;
        if ((random_task_state_vector - qs_task_state_vector).norm() < 0.045){
            path_it++;
            if(path_it == astar_path.end())
            {
                reached_state_matrix = qs_matrix;
                reached_task_pos = qs_task_state_vector;
                reached_index = qs_old_index;
...
bool DualCBiRRT::plan_task_space_dir_astar(robot_state::RobotState & goal_state, robot_state::RobotState & start_state, planning_scene::PlanningScenePtr& planning_scene_ptr, const std::string & planning_group_name, const robot_state::JointModelGroup* planning_group, const robot_state::JointModelGroup* slave_group, AstarPathFinder * astar_path_finder){
    ...
    for(int count=0; count < _max_planning_times; count++){
        ...
        if(extend_order){
            ...
            if(_random_distribution(_random_engine)){
                ...
                constraint_extend_task_space_dir_astar(random_state_value_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, extend_order, true, a_tree_reached_vector, a_tree_reached_index, astar_path_finder);
            }
            nearest_node_index = near_tree_task_space(a_tree_reached_vector, nearest_node_task_state, nearest_node_matrix, !extend_order);
            if(_random_distribution(_random_engine)){
                constraint_extend_task_space_dir(a_tree_reached_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, !extend_order, false, b_tree_reached_vector, b_tree_reached_index);
                ...
                constraint_extend_task_space_dir_astar(a_tree_reached_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, !extend_order, false, b_tree_reached_vector, b_tree_reached_index, astar_path_finder);
            }

            std::cout<<"a_tree_reached_vector: "<<a_tree_reached_vector.transpose()<<std::endl;
            std::cout<<"b_tree_reached_vector: "<<b_tree_reached_vector.transpose()<<std::endl;

            if((a_tree_reached_vector - b_tree_reached_vector).norm() < 0.045)
            {
                ROS_INFO("Success!!!");
                //先添加前半部分路径点
                size_t back_track = a_tree_reached_index;
            ...
            if(_random_distribution(_random_engine)){
                ...
                constraint_extend_task_space_dir_astar(random_state_value_matrix, nearest_node_matrix, nearest_node_index, b_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, extend_order, true, b_tree_reached_vector, b_tree_reached_index, astar_path_finder);
            }
            nearest_node_index = near_tree_task_space(b_tree_reached_vector, nearest_node_task_state, nearest_node_matrix, !extend_order);
            if(_random_distribution(_random_engine)){
                constraint_extend_task_space_dir(b_tree_reached_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, !extend_order, false, a_tree_reached_vector, a_tree_reached_index);
                ...
                constraint_extend_task_space_dir_astar(b_tree_reached_matrix, nearest_node_matrix, nearest_node_index, a_tree_reached_matrix, planning_group, planning_group_name, planning_scene_ptr, slave_group, slave_joint_pos_bounds, perdex_one_sample, worldFcl, robot, !extend_order, false, a_tree_reached_vector, a_tree_reached_index, astar_path_finder);
            }

            std::cout<<"a_tree_reached_vector: "<<a_tree_reached_vector.transpose()<<std::endl;
            std::cout<<"b_tree_reached_vector: "<<b_tree_reached_vector.transpose()<<std::endl;
            if((a_tree_reached_vector - b_tree_reached_vector).norm() < 0.025)
            {
                ROS_INFO("Success!!!");
                //先添加前半部分路径点
                size_t back_track = a_tree_reached_index;
