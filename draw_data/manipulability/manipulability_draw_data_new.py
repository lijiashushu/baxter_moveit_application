...
for i in range(test_num):
    ...
    # names = ["master_explore", "slave_explore", "master_exploit", "slave_exploit", "mini_explore", "mini_exploit"]
    # for k in range(6):
    #     plt.figure(i)
    #     plt.subplot(3, 2, k+1)
    ...
    names = ["distance", "master_dir_manipulability", "slave_dir_manipulability"]
    for kk in range(3):
        plt.figure(i)
        plt.subplot(2, 2, kk+1)
        plt.title("test"+str(i) + "pose" + str(data_index) + names[kk])
        x = np.linspace(0, datalength, datalength)
        for j in range(4):
            plt.plot(x, pose[j][0: datalength, kk+6], label="pose"+str(j))
        plt.legend(loc="best")
plt.show()
