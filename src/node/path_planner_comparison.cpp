...
//    spinner.start();
    ...
    nh.param("dualrrt/max_planning_time",  max_planning_time, 2.0);
    ...
    std::string dualInformdStarFileDir = "/dual_informed_star/";
    std::ofstream seedFile, dualFile, dualStarFile, dualInformedStarFile;

    TaskSpacePlanner *planner = new TaskSpacePlanner(nh);
    seedFile.open(outputDir + seedFileName, std::ios::trunc);
    for(int i=0; i<test_times; i++){
        ...
        planner->initPara(seed,
                          ...
                          false,
                          false);
        dualFile.open(outputDir + dualFileDir + number + ".txt");
        planner->plan(dualFile);
        dualFile.close();
        ...
        planner->initPara(seed,
                          ...
                          true,
                          false);
        dualStarFile.open(outputDir + dualStarFileDir + number + ".txt");
        planner->plan(dualStarFile);
        dualStarFile.close();
        ...
        planner->initPara(seed,
                          ...
                          true,
                          true);
        dualInformedStarFile.open(outputDir + dualInformdStarFileDir + number + ".txt");
