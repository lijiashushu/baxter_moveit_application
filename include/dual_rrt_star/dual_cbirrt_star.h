...
class DualCBiRRT{
    ...
    double _task_step_size = 0.02; //扩展时一点点扩展，但是增长了0.1m之后才算作一个新的节点，相当于这个step是collision check的step
   ...
   */
    void sampleEllipsoid(double info_cBest);
   ...
   */
    bool extend_check(bool if_tree_a, const Eigen::Vector4d& goal_state_vec, int try_index);
...
public:
    ...
    /**
       ...
       */
    DualCBiRRT(const ros::NodeHandle &nh, std::string base_frame="base");
    ~DualCBiRRT();
    ...
    /**
       *  @brief  plan the path from the start state to the goal state
       *  @param  panningTime  record the total planning time
       *  @param  sampleCounts  record the total sample counts
       *  @param  avgExtendTime  record the average time of extend operate
       *  @param  avgExtendCountPerSample  record the average numbers of extend states in one sample
       *  @return find a feasible path or not
       */
    bool plan(double &panningTime, int &sampleCounts, double &avgExtendTime, double &avgExtendCountPerSample);
    ...
    /**
       ...
       */
    void initPara(int seed,  double biProbility, int maxSampleTimes, double taskStep, size_t constrainIndex,
